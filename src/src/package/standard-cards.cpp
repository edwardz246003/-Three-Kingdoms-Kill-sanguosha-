...
bool Slash::isAvailable(const Player *player) const{
    if (player->getMark("imba") && this->getNumber() > player->getMark("imba"))
        return false;
...
bool Slash::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const{
    ...
    if(inherits("WushenSlash") || Self->hasSkill("zhuanzhu")){
        distance_limit = false;
    }
...
bool Snatch::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const{
    ...
    if(Self->distanceTo(to_select) > 1 && !Self->hasSkill("qicai") && !Self->hasSkill("boxue"))
        return false;

    if(Self->hasSkill("boxue") && Self->distanceTo(to_select) > 2)
        return false;
...
YingbaCard::YingbaCard(Suit suit, int number)
    :DelayedTrick(suit, number)
{
    setObjectName("YingbaCard");
    target_fixed = false;
...
bool YingbaCard::targetFilter(const QList<const Player *> &targets, const Player *to_select, const Player *Self) const
{
    if(!targets.isEmpty())
        return false;
...
void YingbaCard::takeEffect(ServerPlayer *target) const{

    target->gainMark("imba", this->getNumber());
}
