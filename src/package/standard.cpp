...
void EquipCard::use(Room *room, ServerPlayer *source, const QList<ServerPlayer *> &) const{
    ...
    if(source->hasSkill("yingya") && this->location() != ArmorLocation && room->askForSkillInvoke(source, "yingya")){

            QList<ServerPlayer *> targets;
            foreach(ServerPlayer *target, room->getAlivePlayers()){
                if(source->canSlash(target))
                    targets << target;
            }

            ServerPlayer *target = room->askForPlayerChosen(source, targets, "yingya-slash");
...
void DelayedTrick::use(Room *room, ServerPlayer *source, const QList<ServerPlayer *> &targets) const{
    if(target_fixed)
        ...
        if (target->getMark("yingba") > 0) target->loseMark("yingba", 1);
        if(this->objectName() == "YingbaCard"){
            target->gainMark("yingba", 1);
        }
...
void DelayedTrick::onEffect(const CardEffectStruct &effect) const{
    ...
    if(!movable && this->objectName() != "YingbaCard")
        room->throwCard(this);
    ...
    }else if(this->objectName() == "YingbaCard" && judge_struct.card->getNumber() <= this->getNumber()){
        room->throwCard(this);
        takeEffect(effect.to);
    }else if(movable || this->objectName() == "YingbaCard"){
    ...
    :Package("standard")
{
    ...
    patterns["analeptic"] = new NamePattern("analeptic");
}
