...
Shite::Shite(Suit suit, int number):BasicCard(suit, number){
    setObjectName("Shite");

    target_fixed = true;
}

QString Shite::getSubtype() const{
    return "disgusting_card";
}

void Shite::use(Room *room, ServerPlayer *source, const QList<ServerPlayer *> &targets) const{
    room->throwCard(this);
    room->cardEffect(this, source, source);
}

void Shite::onEffect(const CardEffectStruct &effect) const{
