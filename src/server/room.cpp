...
void Room::display(JudgeStruct &display_struct){
    Q_ASSERT(display_struct.who != NULL);

    JudgeStar display_star = &display_struct;

    QVariant data = QVariant::fromValue(display_star);

    thread->trigger(StartJudge, display_star->who, data);

    thread->trigger(FinishJudge, display_star->who, data);
}
...
void Room::judge(JudgeStruct &judge_struct){
    ...
    Room *Room1 = judge_star->who->getRoom();
    ServerPlayer *zhoushen = Room1->findPlayerBySkillName("shenhui");
    if(zhoushen && !zhoushen->isKongcheng() && !zhoushen->getAI())
    {

        Room *room=zhoushen->getRoom();
        QString prompt = QString("@shenhui2:%1:%2:%3").arg(judge_struct.who->getGeneralName()).arg(judge_struct.card->getSuitString()).arg(judge_struct.card->objectName());
        if(room->askForCard(zhoushen,".H",prompt))
        {
        zhoushen->askForSkillInvoke("shenhui");
        LogMessage log;
        log.type = "$shenhui2";
        log.to << judge_struct.who;
        log.from = zhoushen;
        room->sendLog(log);
        room->throwCard(judge_struct.card);
...
bool Room::askForBaqi1(ServerPlayer *player, const QString &skill_name, const QVariant &data){
    bool invoked;
    AI *ai = player->getAI();
    if(ai){
        thread->delay(Config.AIDelay);
        ...
        player->invoke("askForBaqi1", skill_name);
        getResult("invokeSkillCommand", player);

        if(result.isEmpty())
            return askForBaqi1(player, skill_name); // recursive call;
...
bool Room::askForBaqi2(ServerPlayer *player, const QString &skill_name, const QVariant &data){
    bool invoked;
    AI *ai = player->getAI();
    if(ai){
        thread->delay(Config.AIDelay);
        ...
        player->invoke("askForBaqi2", skill_name);
        getResult("invokeSkillCommand", player);
...
bool Room::isCanceled(const CardEffectStruct &effect){
    if(effect.card->getTypeId() == Card::Basic)
    {
    Room *Room1=effect.to->getRoom();
    ServerPlayer *zhoushen = Room1->findPlayerBySkillName("shenhui");
    if(zhoushen && !zhoushen->isKongcheng() && !zhoushen->getAI())
    {
       Room *room=zhoushen->getRoom();
       QString prompt = QString("@shenhui1:%1:%2:%3").arg(effect.from->getGeneralName()).arg(effect.to->getGeneralName()).arg(effect.card->objectName());
       if(room->askForCard(zhoushen,".H",prompt))
       {
        zhoushen->askForSkillInvoke("shenhui");
        LogMessage log;
        log.type = "$shenhui";
        log.card_str = effect.card->getEffectIdString();
        log.to << effect.from;
        log.from = zhoushen;
        room->sendLog(log);
        return true;
    ...
    if(trick && trick->objectName() != "YingbaCard"){
...
void Room::gainMaxHp(ServerPlayer *player, int gain){
    player->setMaxHP(qMax(player->getMaxHP() + gain, 0));
...
QString Room::askForSuits(ServerPlayer *player){
    /*AI *ai = player->getAI();
    if(ai)
        return ai->askForSuits();*/

    player->invoke("askForSuits");
    getResult("chooseSuitCommand", player);

    if(result.isEmpty())
        return askForSuits(player);

    if(result == ".")
        return "(.*):(club|heart|diamond):(.*)";
    else if(result == "spade")
            return "(.*):(club|heart|diamond):(.*)";
        else if(result == "club")
            return "(.*):(spade|heart|diamond):(.*)";
        else if(result == "heart")
            return "(.*):(spade|club|diamond):(.*)";
        else
...
void Room::doKaihei(ServerPlayer *siniang, ServerPlayer *target){
    if(siniang->getState() != "online"){
        // throw the first card
        QList<const Card *> cards = target->getHandcards();
        showCard(target, cards.first()->getEffectiveId());
        thread->delay();
        return;
    }
    ...
    siniang->invoke("doKaihei", QString("%1:%2").arg(target->objectName()).arg(handcards_str.join("+")));
    getResult("replyGongxinCommand", siniang);
