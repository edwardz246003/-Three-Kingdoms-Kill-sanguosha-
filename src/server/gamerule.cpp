...
void GameRule::onPhaseChange(ServerPlayer *player) const{
    ...
    case Player::Draw: {
            ...
            if(player->hasSkill("zhazha1")) num = 8;
    ...
    case Player::Finish: {
            if(player->getMark("shourendage"))
                player->drawCards(player->getMark("shourendage"));
            break;
        }
...
bool GameRule::trigger(TriggerEvent event, ServerPlayer *player, QVariant &data) const{
    ...
    if(room->getTag(QLatin1String("SkipGameRule")).toBool()){
    ...
    case SlashProceed:{
            ...
            if(jink){
                    ...
                    {
                        Room *Room2=zhoushen->getRoom();
                        QString prompt = QString("@shenhui3:%1").arg(effect.to->getGeneralName());
                        if(Room2->askForCard(zhoushen,".H",prompt))
                        {
                        ...
                        log.from = zhoushen;
                        room->sendLog(log);
                        jink = room->askForCard(effect.to, "jink", "slash-jink:" + slasher);
                        }
                    }
                }
    ...
    case FinishJudge:{
            JudgeStar judge = data.value<JudgeStar>();
            if(!player->hasFlag("yaomei") && !player->hasFlag("xingchen")){
                room->throwCard(judge->card);
                if(judge->card->objectName() == "shit")
                    room->loseHp(judge->who, 1);
            }

            if(player->hasFlag("xingchen") && judge->isBad()){
                room->throwCard(judge->card);
                if(judge->card->objectName() == "shit")
                    room->loseHp(judge->who, 1);
            }
