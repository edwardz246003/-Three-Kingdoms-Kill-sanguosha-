...
QStringList Engine::getKingdoms() const{
    ...
    if(kingdoms.isEmpty())
        kingdoms << "wei" << "shu" << "wu" << "qun" << "god" << "dan" << "shen" << "ji" << "ren" << "shi";
...
QColor Engine::getKingdomColor(const QString &kingdom) const{
    ...
    if(color_map.isEmpty()){
        ...
        color_map["dan"] = QColor(0x80, 0x80, 0x0);
        color_map["shen"] = QColor(0xFF, 0x0, 0x0);
        color_map["ji"] = QColor(0x0, 0xFF, 0xFF);
        color_map["ren"] = QColor(0x80, 0x0, 0xFF);
    }
