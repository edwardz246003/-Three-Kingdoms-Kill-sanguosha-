...
int Player::getAttackRange() const{
    ...
    int n = 0;
    if(getMark("shenju"))
        n = this->getMark("shenju");
    if(weapon)
        return n + weapon->getRange();
...
int Player::distanceTo(const Player *other) const{
    ...
    if(this->hasSkill("bisuo")) distance++;
    if(other->hasSkill("bisuo")) distance++;
    if(this->hasSkill("tifei")) distance = 1;
    if(other->hasSkill("tifei")) distance = 1;
...
int Player::getMaxCards() const{
    ...
    return qMax(hp,0) + extra + juejing + xueyi + shenwei + jiejian;
}
...
void Player::addDelayedTrick(const Card *trick){
    judging_area << trick;
    if (this->getMark("yingba") > 0){
        DelayedTrick *trickcard = new YingbaCard(trick->getSuit(), trick->getNumber());
        trickcard->addSubcard(trick->getId());
        delayed_tricks << trickcard;
    }
    else delayed_tricks << DelayedTrick::CastFrom(trick);
}
...
bool Player::canSlash(const Player *other, bool distance_limit) const{
    ...
    if(other->hasSkill("yinpan") && this->getHp() > other->getHp())
        return false;
