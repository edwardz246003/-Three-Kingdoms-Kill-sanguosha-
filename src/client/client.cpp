    ...
    :QObject(parent), refusable(true), status(NotActive), alive_count(1), swap_pile(0)
{
    ...
    callbacks["doKaihei"] = &Client::doKaihei;
    ...
    callbacks["askForSuits"] = &Client::askForSuits;
    ...
    callbacks["askForBaqi1"] = &Client::askForBaqi1;
    callbacks["askForBaqi2"] = &Client::askForBaqi2;
...
void Client::askForBaqi1(const QString &skill_name){
    bool auto_invoke = frequent_flags.contains(skill_name);
    if(auto_invoke){
        invokeSkill(QDialog::Accepted);
        return;
    ...
    QString text = tr("Do you want to invoke skill Baqi1 ?").arg(dialog->windowTitle());
...
void Client::askForBaqi2(const QString &skill_name){
    bool auto_invoke = frequent_flags.contains(skill_name);
    if(auto_invoke){
        invokeSkill(QDialog::Accepted);
        return;
    ...
    QString text = tr("Do you want to invoke skill Baqi2 ?").arg(dialog->windowTitle());
...
void Client::askForSuits(const QString &){
    delete ask_dialog;
...
void Client::doKaihei(const QString &gongxin_str){
    QRegExp rx("(\\w+)(!?):(.+)");
    if(!rx.exactMatch(gongxin_str))
        return;
    ...
    emit kaihei(card_ids, enable_heart);
    setStatus(AskForKaihei);
}
