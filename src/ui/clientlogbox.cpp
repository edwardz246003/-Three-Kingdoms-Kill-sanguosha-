        ...
        QString arg2)
{
    ...
    if(!card_str.isEmpty()){
        ...
        if(card_str=="96"||card_str=="95"||card_str.contains("collateral"))
        {
            RoomSceneInstance->showIndicator(from_general, tos[0]);
            RoomSceneInstance->showIndicator(tos[0], tos[1]);

        }
        else {foreach(QString to, tos){
