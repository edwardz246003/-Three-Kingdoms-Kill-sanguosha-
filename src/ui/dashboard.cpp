...
void Dashboard::installDelayedTrick(CardItem *card){
    ...
    if (Self->getMark("yingba") > 0){
        //Self->marks.remove("yingba");
        trick = new YingbaCard(card->getCard()->getSuit(), card->getCard()->getNumber());
        delayed_tricks << QPixmap(trick->getIconPath());
    }else{
    trick = DelayedTrick::CastFrom(card->getCard());
    delayed_tricks << QPixmap(trick->getIconPath());
    }
